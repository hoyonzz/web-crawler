...
```mermaid
graph TD
    A1["GitHub Actions cron (매일 자동 실행)"] --> B["Selenium WebDriver"]
    A2["로컬 수동 실행 · Docker"] --> B
    B --> C1["잡코리아 / 점핏"]
    B --> C2["원티드 (GHA 실행 시 CloudFront 403 차단 — 로컬에서만 수집)"]
    C1 -->|원천 데이터| D["Python ETL"]
    C2 -->|원천 데이터| D
...
- GitHub Actions cron 자동화가 가동 중이며, 두 실행 경로의 파이프라인은 동일하다. 다만 데이터센터 IP에 대한 플랫폼 측 봇 차단(CloudFront 403)으로 원티드는 GHA 환경에서 수집이 실패하며, 로컬 실행 시에는 3개 플랫폼 모두 정상 수집된다.
...
- **재시도·복구:** 일시적 장애(5xx·타임아웃)는 tenacity **점증형 재시도(4→8→10초, 상한 적용)**로 재시도하고, 최종 실패 건은 다음 실행에서 자동 재처리된다(중복 확인 구조의 부수 효과).
...
> **설정:** 판정 기준(키워드 가중치, 조건부 제외 룰, 컷오프)은 `config/job_filter_config.yaml`에서 관리하며 코드 수정 없이 조정한다.
> 
> LLM 공급자는 환경변수 한 줄로 Gemini ↔ NVIDIA NIM 전환된다. API 키는 `.env`로 주입한다.
...
| LLM 판정 | — | 약 415회 (구조화 출력 성공률 98% 이상, 실패분은 재실행 자동 재처리) |
...
- **v1 가동 완료 (2025.01 ~ 2025.12):** GitHub Actions 스케줄로 129회 무중단 실행.
  3개 플랫폼 수집 → 2단계 필터 → LLM 판정 → Notion 적재의 전체 사슬 검증

<img width="1429" height="969" alt="GitHub Actions 129회 무중단 실행 이력" src="https://github.com/user-attachments/assets/b8a24c39-cb51-4b87-a88d-5d6d2f6b40ba" />

- **1단계 고도화 완료 (2026-07):** 크롤러 전수 재검증·점핏 교체, 조건부 제외 룰, Pydantic 구조화 판정 + 룰 파서 + 격리 설계, LLM 이원화, Docker 재현성(의존성 핀), 3개 플랫폼 949건 백필
- **현재:** 일일 운영 중 (Gemini 모드) — 잡코리아·점핏은 GHA 자동 수집, 원티드는 로컬 실행으로 보완
